...
For this case, we choose:
    Ra = 1708 <==> nu = 3.389e-02.

Thus we expect this case to be convectively unstable. Ra/Ra_crit =1, so we 
expect the critical transition -- i.e. marginal stability.


# Current status
Be mindful of the requirement to keep the diffusion term stable: 
    nuEff*dt/(dx^2) < 0.5
    so for nuEff = 3.389e-02,
    dx^2/dt > 2*nuEff
            > 6.778e-02; for dx = 1e-01 (100x10 res for a.r.=10),
         dt < 1e-2/6.778e-02
            < 1.5e-01
