...
mkdir 0
cp -r init_0/* 0
# set linear theta profile (ExnerFoam)
setAnalyticTracerField -name theta -tracerDict theta_tracerFieldDict

# set linear T profile (buoyantPimpleFoam)
setAnalyticTracerField -name T -tracerDict T_tracerFieldDict
...
# add Gaussian random noise to theta or T field 
## (is it consistent to only do this for this field?)
# for exnerFoam
cp system/randomise_theta system/randomise  
...
# for buoyantPimpleFoam
cp system/randomise_T system/randomise
postProcess -func randomise -time 0
mv 0/T 0/T_init
mv 0/TRandom 0/T
