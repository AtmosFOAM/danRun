...
For this case, we choose:
    Ra = 1600 <==> nu = 3.132e-02.

Thus we expect this case to be convectively unstable. Ra/Ra_crit ~= 1.17, so we 
expect slight instability, and heance steady convective rolls.


# Current status
Be mindful of the requirement to keep the diffusion term stable: 
    nuEff*dt/(dx^2) < 0.5
    so for nuEff = 3.132e-02,
    dx^2/dt > 2*nuEff
            > 6.264e-02; for dx = 1e-01 (100x10 res for a.r.=10),
         dt < 1e-2/6.264e-02
            < 1.6e-01
