    ...
    if createFields == True:
        if append == True:
            ...
            if times_OLD[-1] + increment != times[0]:
                ...
                print("Loading field time series (times %f to %f): " % (times_OLD[0], times_OLD[-1]))
                print("Loading b")
                ...
                buoyancy, zNew = createFieldTimeSeries("b",times, nx, nz, returnZ=True, X=L, append=True, appendField=buoyancy_OLD)
                gradB = createFieldTimeSeries("grad(b)",times, nx, nz, X=L, append=True, appendField=gradB_OLD)
                u = createFieldTimeSeries("u",times, nx, nz, X=L, append=True, appendField=u_OLD)
            ...
            buoyancy, zNew = createFieldTimeSeries("b",times, nx, nz, returnZ=True, X=L)
            gradB = createFieldTimeSeries("grad(b)",times, nx, nz, X=L)
...
def profileMinMax(field, z, bulkMax=False):
    ...
    bulkMax = False:
    return array of (upperHalfMax, loc, lowerHalfMax, loc, bulkMin, loc, BLdepthUp, BLdepthLo)
    for each time.
    
    bulkMax = True:
    return array of (upperHalfMax, loc, lowerHalfMax, loc, bulkMax, loc, BLdepthUp, BLdepthLo)
    """
    if len(np.shape(field)) != 2:
        ...
        for it in range(Nt):
            ...
            # BL depth from gradient at wall
            grad = np.gradient(field[it,:], z, edge_order=2)
            profileMinMax[it,6] = (1 - profileMinMax[it,0]) / grad[-1]    # upper BL depth
            profileMinMax[it,7] = profileMinMax[it,2] / grad[0]     # lower BL depth
