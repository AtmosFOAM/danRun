...
# Write correct level interfaces to horizontalMeanDict
#nLevels=801
#increment=$(echo "1 / $(($nLevels-1))" | bc -l )
#levels=[$(seq 0 $increment 1)]
#find . -type f -name horizontalMeanDict_test -print0 | xargs -0 sed -i '' -e "s/\(levelInterfaces\).*\(\(\).*\(\);\)/\1\t\($nLevels\)\2\n\($levels\)\n\3/g"
