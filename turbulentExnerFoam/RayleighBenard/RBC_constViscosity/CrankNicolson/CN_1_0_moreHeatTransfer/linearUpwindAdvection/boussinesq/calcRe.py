...
def main():
    ...
    for it, time in enumerate(times):
        ...
        if partitioned == True:
            ...
            # re-normalise
            print(len(times[0:it]))
            print(len(times[0:it+1]))
            uMag_horAv_timeAv   = uMag_horAv_timeAv * ( float(len(times[0:it])) / float(len(times[0:it+1])) )
            ...
            # update 
            uMag_horAv_timeAv   += uMag_horAv / float(len(times[0:it+1]))
            ...
            with open("ReTimeSeries.txt", "a") as f:
                print("%lf\t %lf\t %lf\t " #%lf\t %lf\t %lf\t %lf\t %lf\t %lf\t %lf\t %lf\t %lf\t %lf\t " 
                      % (time, 
                         np.max(uMag_horAv), 
                         np.max(uMag_horAv_timeAv)), file=f)
                         #np.max(uxMag_horAv),
                         #np.max(uxMag_horAv_timeAv),
                         #np.max(uzMag_horAv),
                         #np.max(uzMag_horAv_timeAv),
