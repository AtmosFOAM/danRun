...
for time in 0 0.005 0.01; do
    for var in Exner theta Uf; do
        sumFields $time $var.diff $time $var ../Ra_1e+05_multiFluidFoam_X1_Y50_constThetaBC/$time $var -scale0 -1
    done
done
gmtFoam ExnerDiff_multiVsSingle
gmtFoam thetaDiff_multiVsSingle

# Differences between ExnerFoamAdv and turbulentExnerFoam (Ex minus turb)
for time in 0 0.005 0.01; do
    for var in Exner theta Uf; do
        sumFields $time $var.diff_ExVsTurb $time $var ../Ra_1e+05_turbExnerFoam_X1_Y50_constThetaBC/$time $var -scale0 -1
    done
done
gmtFoam ExnerDiff_ExVsTurb
gmtFoam thetaDiff_ExVsTurb

# Differences from initial conditions
for time in 0.005 0.01; do
    for var in Exner theta Uf; do
    sumFields $time $var.diff_initConds 0 $var $time $var -scale0 -1
    done
done
gmtFoam ExnerDiff
gmtFoam thetaDiff
