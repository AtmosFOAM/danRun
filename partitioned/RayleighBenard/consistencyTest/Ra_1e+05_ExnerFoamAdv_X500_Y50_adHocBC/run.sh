...
for time in 0 0.005 0.01; do
    for var in Exner theta Uf; do
        sumFields $time $var.diff $time $var ../Ra_1e+05_multiFluidFoam_X500_Y50_adHocBC/$time $var -scale0 -1
    done
done
...
for time in 0 0.005 0.01; do
    for var in Exner theta Uf; do
        sumFields $time $var.diff_ExVsTurb $time $var ../Ra_1e+05_turbExnerFoam_X500_Y50_adHocBC/$time $var -scale0 -1
    done
done
