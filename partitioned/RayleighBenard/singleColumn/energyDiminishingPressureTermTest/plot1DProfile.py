...
def plotPressurePerturbation(refDir, PScale, zScale):
    ...
    Ptilde = - (sigma_rising*mean_rising[:,1] + sigma_falling*mean_falling[:,1])
    
    # conditioned figure
    plt.figure()
    # means
    # subtract vertical mean pressure because absolute value of pressure doesn't matter
    plt.plot( -zMean + mean[:,1] + Ptilde + mean_rising[:,1] + mean_falling[:,1], mean[:,0], 'k', label="total")
    plt.plot( -zMean + mean[:,1] + Ptilde + mean_rising[:,1],mean_rising[:,0],'r', label=r"$P_1$")
    plt.plot( -zMean + mean[:,1] + Ptilde + mean_falling[:,1],mean_falling[:,0],'b', label=r"$P_0$")
    # reference profiles & stddevs.
