...
FROM=../Ra_1e+05_multiFluidFoam_hiRes/100

values=`awk '{if(NR>1)print $2}' $FROM/horizontalMean_falling_rho_Exner.dat | paste -s`
sed -i 's/REPLACE/'"$values"'/g' 0/sigma.stable

values=`awk '{if(NR>1)print $2}' $FROM/horizontalMean_rising_rho_Exner.dat | paste -s`
sed -i 's/REPLACE/'"$values"'/g' 0/sigma.buoyant

values=`awk '{if(NR>1)print $4}' $FROM/horizontalMean_falling_rho_theta.dat | paste -s`
sed -i 's/REPLACE/'"$values"'/g' 0/theta.stable

values=`awk '{if(NR>1)print $4}' $FROM/horizontalMean_rising_rho_theta.dat | paste -s`
sed -i 's/REPLACE/'"$values"'/g' 0/theta.buoyant

values=`awk '{if(NR>1)print "("0, 0, $4")"}' $FROM/horizontalMean_falling_rho_uz.dat | paste -s`
sed -i 's/REPLACE/'"$values"'/g' 0/u.stable

values=`awk '{if(NR>1)print "("0, 0, $4")"}' $FROM/horizontalMean_rising_rho_uz.dat | paste -s`
sed -i 's/REPLACE/'"$values"'/g' 0/u.buoyant

values=`awk '{if(NR>1)print $4}' $FROM/horizontalMean_rho_Exner.dat | paste -s`
sed -i 's/REPLACE/'"$values"'/g' 0/Exner
